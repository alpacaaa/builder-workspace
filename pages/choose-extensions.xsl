...
<div class="content">
	...
	<xsl:if test="false() and ($details/details[@web-id = $url-show]) and ($url-what = 'overview')">
		
		<div class="overview overlay">
			<xsl:copy-of select="$details/details[@web-id = $url-show]/overview/*" />
		</div>
	...
	<xsl:variable name="elements" select="//store/key[@handle = 'Extension']/key" />
	<a class="next-step">
		<xsl:attribute name="href">
			<xsl:value-of select="concat($root, '/')" />
			
			<xsl:choose>
				<xsl:when test="count($elements) = count($elements//key[@handle = 'source'])">
					<xsl:text>choose-utilities</xsl:text>
				</xsl:when>
				<xsl:otherwise>
					<xsl:text>choose-extensions/to-be-installed</xsl:text>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:attribute>
		
		<xsl:text>Go on</xsl:text>
	</a>
</xsl:template>
	...
	<xsl:for-each select="xhtml:li/xhtml:a">
		<li>
			...
			<xsl:attribute name="href">
				...
				<xsl:value-of select="str:replace(@href, '/download/extensions/', '')" />
			</xsl:attribute>
				<xsl:value-of select="." />
			</a>
		</li>
...
<xsl:template match="//xhtml:ul[@class='listings-row']/xhtml:li">
	...
	<xsl:variable name="favourited">
		<xsl:choose>
			<xsl:when test="//objects-extensions/web-id[@handle = $web-id] != ''">
				<xsl:value-of select="//objects-extensions/web-id[@handle = $web-id]//favourited" />
			</xsl:when>
	...
	<xsl:variable name="id">
		<xsl:choose>
			<xsl:when test="//objects-extensions/web-id[@handle = $web-id] != ''">
				<xsl:value-of select="//objects-extensions/web-id[@handle = $web-id]/entry/@id" />
			</xsl:when>
